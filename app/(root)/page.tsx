    ...
    <section className='w-full py-10 md:py-20 px-4'>
      <div className='max-w-6xl mx-auto space-y-8'>
        ...
        <div className='text-center space-y-3 max-w-3xl mx-auto'>
          ...
          <div className='flex flex-wrap items-center justify-center gap-3 pt-2'>
            <span className='inline-flex items-center gap-1.5 text-xs font-semibold text-orange-700 bg-orange-50 px-3 py-1.5 rounded-full border border-orange-200'>
              <svg className='h-3.5 w-3.5' fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={2.5}><path strokeLinecap='round' strokeLinejoin='round' d='M5 13l4 4L19 7' /></svg>
              Auto Invoicing
            </span>
            <span className='inline-flex items-center gap-1.5 text-xs font-semibold text-orange-700 bg-orange-50 px-3 py-1.5 rounded-full border border-orange-200'>
        ...
        <div className='grid gap-4 md:gap-6 md:grid-cols-2'>
          <div className='md:col-span-2 group relative rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-500 hover:scale-[1.01] bg-gradient-to-r from-orange-100 via-amber-50 to-rose-100 border border-orange-200/60 shadow-xl'>
            <div className='absolute top-4 right-4 md:top-6 md:right-6'>
              <span className='inline-flex items-center gap-1 bg-white text-orange-600 text-[10px] md:text-xs font-bold px-3 py-1.5 rounded-full border border-orange-200'>
                <span className='relative flex h-2 w-2'>
            ...
            <div className='relative p-6 md:p-8 space-y-4 md:space-y-6'>
              <div className='space-y-2'>
                <h3 className='text-2xl md:text-3xl font-bold text-black'>From Lead to Payment</h3>
                <p className='text-black/60 text-sm md:text-base'>Every job follows this flow — automatically</p>
              </div>

              {/* Mobile layout */}
              <div className='lg:hidden space-y-2'>
                {[
                  [{ label: 'Lead Comes In', sub: 'Marketplace or Direct', border: 'border-rose-300', color: 'text-rose-600' }, { label: 'Send Quote', sub: 'Built-in Quote Builder', border: 'border-orange-300', color: 'text-orange-600' }],
                  [{ label: 'Customer Accepts', sub: 'One Click Approval', border: 'border-amber-300', color: 'text-amber-600' }, { label: 'Contract Auto-Sent', sub: 'E-Signature Ready', border: 'border-emerald-300', color: 'text-emerald-600' }],
                  [{ label: 'Customer Signs', sub: 'Legally Binding', border: 'border-emerald-300', color: 'text-emerald-600' }, { label: 'Job Created', sub: 'Auto-Scheduled', border: 'border-blue-300', color: 'text-blue-600' }],
                  [{ label: 'Upload Photos', sub: 'Before / During / After', border: 'border-violet-300', color: 'text-violet-600' }, { label: 'Track Progress', sub: 'Real-Time Updates', border: 'border-violet-300', color: 'text-violet-600' }],
                  [{ label: 'Job Complete', sub: 'Customer Notified', border: 'border-cyan-300', color: 'text-cyan-600' }, { label: 'Invoice Sent', sub: 'Auto-Generated', border: 'border-cyan-300', color: 'text-cyan-600' }],
                  [{ label: 'Get Paid', sub: 'Direct to Bank', border: 'border-green-300', color: 'text-green-600' }, { label: 'Get Reviews', sub: 'Auto-Requested', border: 'border-green-300', color: 'text-green-600' }],
                ].map((row, i) => (
                  <div key={i} className='flex items-center gap-1 sm:gap-2'>
                    <div className={`flex-1 bg-white/80 backdrop-blur-sm rounded-lg px-2 py-2 border-2 ${row[0].border} text-center shadow-sm`}>
                      <div className={`${row[0].color} text-[10px] sm:text-xs font-bold`}>{row[0].label}</div>
                      <div className='text-black/70 text-[8px] sm:text-[10px] mt-0.5 font-semibold'>{row[0].sub}</div>
                    </div>
                    <svg className='h-4 w-4 text-orange-400 shrink-0' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                    <div className={`flex-1 bg-white/80 backdrop-blur-sm rounded-lg px-2 py-2 border-2 ${row[1].border} text-center shadow-sm`}>
                      <div className={`${row[1].color} text-[10px] sm:text-xs font-bold`}>{row[1].label}</div>
                      <div className='text-black/70 text-[8px] sm:text-[10px] mt-0.5 font-semibold'>{row[1].sub}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className='hidden lg:block space-y-4'>
                ...
                <div className='flex items-center justify-between gap-3'>
                  {[
                    { label: 'Lead Comes In', sub: 'Marketplace or Direct', border: 'border-rose-300', color: 'text-rose-600' },
                    { label: 'Send Quote', sub: 'Built-in Quote Builder', border: 'border-orange-300', color: 'text-orange-600' },
                    { label: 'Customer Accepts', sub: 'One Click Approval', border: 'border-amber-300', color: 'text-amber-600' },
                    { label: 'Contract Auto-Sent', sub: 'E-Signature Ready', border: 'border-emerald-300', color: 'text-emerald-600' },
                    { label: 'Customer Signs', sub: 'Legally Binding', border: 'border-emerald-300', color: 'text-emerald-600' },
                  ].map((node, i, arr) => (
                    <div key={node.label} className='flex items-center gap-3 flex-1'>
                      <div className={`bg-white/80 backdrop-blur-sm rounded-lg px-4 py-3 border-2 ${node.border} text-center min-w-[100px] flex-1 shadow-sm`}>
                        <div className={`${node.color} text-xs font-bold`}>{node.label}</div>
                        <div className='text-black/70 text-[10px] mt-0.5 font-semibold'>{node.sub}</div>
                      </div>
                      {i < arr.length - 1 && (
                        <svg className='h-3 w-5 text-orange-400 shrink-0' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className='flex justify-end pr-[50px]'>
                  <div className='flex flex-col items-center'>
                    <div className='w-0.5 h-4 border-l-2 border-dashed border-orange-300'></div>
                    <svg className='h-3 w-5 text-orange-400 rotate-90' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                  </div>
                </div>
                ...
                <div className='flex items-center justify-between gap-3'>
                  {[
                    { label: 'Get Reviews', sub: 'Auto-Requested', border: 'border-green-300', color: 'text-green-600' },
                    { label: 'Get Paid', sub: 'Direct to Bank', border: 'border-green-300', color: 'text-green-600' },
                    { label: 'Invoice Sent', sub: 'Auto-Generated', border: 'border-cyan-300', color: 'text-cyan-600' },
                    { label: 'Job Complete', sub: 'Customer Notified', border: 'border-cyan-300', color: 'text-cyan-600' },
                    { label: 'Job Auto-Created', sub: 'Scheduled & Tracked', border: 'border-blue-300', color: 'text-blue-600' },
                  ].map((node, i, arr) => (
                    <div key={node.label} className='flex items-center gap-3 flex-1'>
                      {i > 0 && (
                        <svg className='h-4 w-4 text-orange-400 shrink-0 rotate-180' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                      )}
                      <div className={`bg-white/80 backdrop-blur-sm rounded-lg px-4 py-3 border-2 ${node.border} text-center min-w-[100px] flex-1 shadow-sm`}>
                        <div className={`${node.color} text-xs font-bold`}>{node.label}</div>
                        <div className='text-black/70 text-[10px] mt-0.5 font-semibold'>{node.sub}</div>
                ...
                <div className='flex justify-start pl-[50px]'>
                  <div className='flex flex-col items-center'>
                    <div className='w-0.5 h-4 border-l-2 border-dashed border-orange-300'></div>
                    <svg className='h-4 w-4 text-orange-400 rotate-90' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                  </div>
                </div>

                {/* Row 3: Extras */}
                <div className='flex items-center gap-3'>
                  <div className='bg-white/80 backdrop-blur-sm rounded-lg px-4 py-3 border-2 border-violet-300 text-center min-w-[100px] shadow-sm'>
                    <div className='text-violet-600 text-xs font-bold'>Upload Photos</div>
                    <div className='text-black/70 text-[10px] mt-0.5 font-semibold'>Before / During / After</div>
                  </div>
                  <svg className='h-4 w-4 text-orange-400' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                  <div className='bg-white/80 backdrop-blur-sm rounded-lg px-4 py-3 border-2 border-violet-300 text-center min-w-[100px] shadow-sm'>
                    <div className='text-violet-600 text-xs font-bold'>Change Orders</div>
                    <div className='text-black/70 text-[10px] mt-0.5 font-semibold'>Scope & Price Updates</div>
                  </div>
                  <svg className='h-4 w-4 text-orange-400' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z' clipRule='evenodd' /></svg>
                  <div className='bg-white/80 backdrop-blur-sm rounded-lg px-4 py-3 border-2 border-amber-300 text-center min-w-[100px] shadow-sm'>
                    <div className='text-amber-600 text-xs font-bold'>Warranty Tracking</div>
                    <div className='text-black/70 text-[10px] mt-0.5 font-semibold'>Auto Reminders</div>
                  </div>
                </div>
              </div>
            </div>
