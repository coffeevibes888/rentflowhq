...
import { X, Sparkles, ArrowRight, Wrench, Star, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import Link from 'next/link';

const STORAGE_KEY = 'contractor_promo_dismissed';
const DISMISS_DURATION_DAYS = 7;

export default function ContractorPromoModal() {
      ...
      <div className="relative z-10 w-full max-w-lg overflow-hidden rounded-2xl shadow-2xl animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-gradient-to-br from-blue-500 via-cyan-500 to-sky-600 p-6 sm:p-8">
          ...
          <p className="text-white/90 text-center text-base sm:text-lg mb-2 leading-relaxed">
            Want Property Managers, Landlords, and Homeowners to find you first? 
            Sign up now and get early visibility on our brand new Contractor Marketplace!
          </p>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <Star className="h-6 w-6 text-amber-300 mx-auto mb-1" />
              <span className="text-white/90 text-xs font-medium">Top Placement</span>
            </div>
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <Users className="h-6 w-6 text-white mx-auto mb-1" />
              <span className="text-white/90 text-xs font-medium">More Clients</span>
            </div>
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <Sparkles className="h-6 w-6 text-cyan-200 mx-auto mb-1" />
