...
export const HomeownerSidebarContext = createContext<{ isCollapsed: boolean }>({ isCollapsed: false });

export function useHomeownerSidebarState() {
  return useContext(HomeownerSidebarContext);
}

interface HomeownerSidebarWrapperProps {
  children: React.ReactNode;
}

export function HomeownerSidebarWrapper({ children }: HomeownerSidebarWrapperProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    setIsMounted(true);
    const saved = localStorage.getItem('homeowner-sidebar-collapsed');
    if (saved !== null) {
  ...
  const toggleSidebar = () => {
    ...
    localStorage.setItem('homeowner-sidebar-collapsed', String(newState));
  };
  ...
  return (
    <HomeownerSidebarContext.Provider value={{ isCollapsed }}>
      <aside
        className={cn(
      ...
      <style jsx global>{`
        ...
        `}
      `}</style>
    </HomeownerSidebarContext.Provider>
  );
}
